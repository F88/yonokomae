...
## デモリポジトリ: common エンジン + locales + wrappers

重複を減らすため、ローカライズされたデモ実装は共有コアと小さなロケールパック、薄いラッパーで構成されています。

フォルダ構成

- コアエンジン(全ロケールで共有するロジック)
    - `src/yk/repo/demo/common/repositories.demo-common.ts`
- ロケールパック(テキスト、ラベル、ロケールごとの微調整)
    - `src/yk/repo/demo/locales/en.ts`
    - `src/yk/repo/demo/locales/ja.ts`
    - `src/yk/repo/demo/locales/de.ts`
- 薄いラッパー(モード別のエクスポート。中でコアに委譲)
...
- ビジネスロジックは common エンジンに置き、ロケール非依存にします。
- ロケールファイルはコピーと簡単なパラメータのみを持ちます。
- ラッパーは極小で、common エンジンとロケールを import して、そのロケール用の具体実装を構築します。
- Provider ファクトリの配線は従来通りです。選択されたモードに応じて正しいラッパーを import します。

新しいデモロケールの追加手順

1. `src/yk/repo/demo/locales/<lang>.ts` にロケールパックを作成:
...
1. common エンジンにロケールを束ねる薄いラッパーを追加:
...
- ユニットテストは common エンジンが表に出す振る舞いに集中します。ロケールのテストは、適切な文字列/ラベルが表出されることのみを確認します。
- 新しいロケール追加時は、スナップショットではなく明示的な文字列確認を好み、差分を小さく保ちます。

メンテナンス

- 新機能はまず common エンジンに追加し、必要な場合のみ既存ロケールパックを更新します。
- ビジネスロジックをロケールパックやラッパーに置かないでください。
