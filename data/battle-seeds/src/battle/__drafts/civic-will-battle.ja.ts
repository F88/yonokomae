...
const battle = {
  ...
  narrative: {
    ...
    scenario:
      '狛江市では、合併への市民の関心が低いという状況下で、当時の町長が「まちのアイデンティティが失われる」という明確な信念のもと、独立を貫き通した。これは、リーダーシップが地域の運命を決定づけた事例だ。対する与野市では、合併に異議を唱える市民団体があったにもかかわらず、住民投票は実施されず、行政主導で合併が推進された。これは、政治的判断が民意の直接的な表明を上回った事例だ。この戦いは、市民民主主義において、住民の「無関心」と首長の「信念」が、地域の未来をどう形作るかという根源的な問いを投げかける。',
  },
  komae: {
