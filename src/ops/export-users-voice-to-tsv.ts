 ...
 *
 * 実行方法:
 * - 標準出力に出す: `npm run ops:export-users-voice-to-tsv`
 * - ファイルへ出す: `npm run ops:export-users-voice-to-tsv -- out/users-voice.tsv`
 * - 手動実行: `npm run ops:build && node dist/ops/export-users-voice-to-tsv.js [outfile]`
 *
 * 引数:
 ...
 * 例:
 * ```bash
 * npm run ops:export-users-voice-to-tsv -- out/users-voice.tsv
 * ```
 */
